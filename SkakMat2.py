...
class Board(object):
    ...
    def clear_path(self, row, column, dest_row, dest_col):
        ...
        if dest_row in range(0, 8) and dest_col in range(0, 8):
            if row != dest_row and column != dest_col:
                ...
                for i in range(
                    min(1, dest_col - column + 1), max(0, dest_col - column)
                ):
    ...
    def convert(self, piece):
        ...
        if not self.moves - 1 in self.aidanMoves and not self.betaBentMove:
            ctype = input(
                "Well done, Master! What kind of piece would you like? (Rook, Knight, Bishop, Queen)"
            )
...
class Chesspiece(object):
    ...
    shouts = {
        "ordinary": "",
        "attack": "I shall eliminate the enemy at once, Sire!",
        "friendlyfire": "Sire, I refuse to attack my trusted ally!",
        "charge": "Chaaarge!!",
        "passant": "En passant!",
        "castling": "The fortifications have been built, Sire",
        "check": "Sire, we have the enemy king in check!",
    ...
    def move_piece(self, dest_row, dest_col):
        if self.is_alive is False:
            ...
            if not incheck:
                ...
                if (
                    not self.board.betaBentMove
                    and not self.board.moves - 1 in self.board.aidanMoves
                ):
                    print("That move leaves our king unprotected!")
                self.board.reverseMove(
                    self, startrow, startcol, dest_row, dest_col, mtype, k_c, killed
                )
        ...
        if not self.row in [0, 7]:
            if self.color == "W":
                if self.board.state[self.row - 1][self.column] is None:
                    self.board.legal_moves[self.color][self][
                        (self.row - 1, self.column)
                    ] = "ordinary"
                    if self.moves == 0:
                        if self.board.clear_path(
                            self.row, self.column, self.row - 3, self.column
                        ):
                            self.board.legal_moves[self.color][self][
                ...
                if (
                    self.column - 1 >= 0
                    and self.board.state[self.row - 1][self.column - 1] is not None
                ):
                ...
                if self.row == 3:
                    if (
                        self.column - 1 >= 0
                        and self.board.state[self.row - 1][self.column - 1] is None
                        and self.board.state[self.row][self.column - 1] is not None
                    ):
                        opp = self.board.state[self.row][self.column - 1]
                        if (
                            opp.name[1:2] == "P"
                            and opp.moves == 1
                            and self.board.charged_pawns[opp] == self.board.moves - 1
                        ):
                            self.board.legal_moves[self.color][self][
                                (self.row - 1, self.column - 1)
                            ] = "passant"
    ...
    def legal_moves(self):
        for i in range(-7, 7):
            if i + self.row in range(0, 8) and i + self.column in range(0, 8):
                ...
                if self.board.clear_path(
                    self.row, self.column, self.row + i, self.column + i
                ):
                    if dest is None:
                        self.board.legal_moves[self.color][self][
...
class Queen(Chesspiece):
    ...
    def legal_moves(self):
        ...
        for rows in range(0, 8):
            ...
            if dest is None:
                if (
                    self.board.clear_path(self.row, self.column, rows, self.column)
                    == True
                ):
                    self.board.legal_moves[self.color][self][
                        (rows, self.column)
            ...
            if self.color == "W":
                if self.board.WR2.moves == 0 and self.board.clear_path(7, 4, 7, 7):
                    for item in self.board.legal_moves["B"]:
                        if (7, 5) in self.board.legal_moves["B"][item] or (
                            7,
                            6,
                        ) in self.board.legal_moves["B"][item]:
                            break
                    else:
                        self.board.legal_moves[self.color][self][
                            (7, 6)
                ...
                if self.board.WR1.moves == 0 and self.board.clear_path(7, 4, 7, 0):
                    for item in self.board.legal_moves["B"]:
                        if (7, 2) in self.board.legal_moves["B"][item] or (
                            7,
                            3,
                        ) in self.board.legal_moves["B"][item]:
                            break
                ...
                if self.board.BR2.moves == 0 and self.board.clear_path(0, 4, 0, 7):
                    for item in self.board.legal_moves["W"]:
                        if (0, 5) in self.board.legal_moves["W"][item] or (
                            0,
                            6,
                        ) in self.board.legal_moves["W"][item]:
                            break
                    else:
                        self.board.legal_moves[self.color][self][
                            (0, 6)
                ...
                if self.board.BR1.moves == 0 and self.board.clear_path(0, 4, 0, 0):
                    for item in self.board.legal_moves["W"]:
                        if (0, 3) in self.board.legal_moves["W"][item] or (
                            0,
                            2,
                        ) in self.board.legal_moves["W"][item]:
                            break
